...
const gifts: Gift[] = [
  ...
  },
  {
    ...
    imageUrl: 'https://example.com/cesto.jpg',
    color: null,
    description: null,
    preferences: [],
    referenceValue: null,
...
describe('LiveCatalogPage availability priority', () => {
  it('mostra os presentes disponíveis primeiro e deixa todos acessíveis por um filtro simples', () => {
    ...
    expect(screen.getByRole('button', { name: 'Disponíveis (2)' })).toHaveAttribute(
      'aria-pressed',
      'true',
    )
    expect(screen.queryByRole('heading', { name: 'Chaleira' })).not.toBeInTheDocument()
    expect(document.querySelector('img.gift-visual--image')).toHaveAttribute(
      'src',
      'https://example.com/cesto.jpg',
    )
